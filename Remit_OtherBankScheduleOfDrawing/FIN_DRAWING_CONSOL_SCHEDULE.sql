CREATE OR REPLACE PACKAGE                                    FIN_DRAWING_CONSOL_SCHEDULE AS 

   PROCEDURE FIN_DRAWING_CONSOL_SCHEDULE(	inp_str      IN  VARCHAR2,
			out_retCode  OUT NUMBER,
			out_rec      OUT VARCHAR2 );

END FIN_DRAWING_CONSOL_SCHEDULE;
/


CREATE OR REPLACE PACKAGE BODY                                                                                                                 FIN_DRAWING_CONSOL_SCHEDULE AS
...
where
   ...
   and (ctd.tran_sub_type = 'BI' or ctd.tran_sub_type = 'RI')
    ...
    and trim (ctd.tran_id) NOT IN (select trim(CONT_TRAN_ID) from TBAADM.ATD atd
        ...
        and atd.cont_tran_date <= TO_DATE( CAST ( ci_TranDate AS VARCHAR(10) ) , 'dd-MM-yyyy' ) )
  group by sol.br_open_date,ctd.dth_init_sol_id,sol.sol_desc,
bct.br_name,ctd.tran_rmks,ctd.tran_amt,ctd.tran_id)q
order by q.AGD;
  ...
  ------------------------------------------------------------------------------
    ...
    END IF;
     ...
     IF NOT ExtractData%ISOPEN THEN
        ...
        --{
          FETCH	ExtractData
          INTO  v_sol_id, v_AGD_BR_name, v_other_Br_name, v_payee_name,v_drawing_amt,v_Commission,v_open_date,v_tran_id;
      ------------------------------------------------------------------------------
